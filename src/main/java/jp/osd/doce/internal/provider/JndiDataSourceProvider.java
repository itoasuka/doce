...
public class JndiDataSourceProvider implements Provider<DataSource> {
    ...
    private boolean useLocalTransaction() {
        ...
        case AUTO:
            if (jndiTransactionName != null) {
                ...
                try {
                    LOGGER.debug(MessageCodes.DG006, DEFAULT_JNDI_TRANSACTION_NAME);
                    context.lookup(DEFAULT_JNDI_TRANSACTION_NAME);
                    // 取得できるのならばローカルトランザクションは使わない
                    LOGGER.debug(MessageCodes.DG008);
                    result = false;
                } catch (NamingException e) {
                    // 例外がでるようならローカルトランザクションを使うしかない
                    LOGGER.debug(MessageCodes.DG007, DEFAULT_JNDI_TRANSACTION_NAME);
                    result = true;
                }
            }
            break;
    ...
    public void setJndiTransactionName(
            @Named(JNDI_USER_TRANSACTION) String jndiTransactionName) {
        LOGGER.debug(MessageCodes.DG002, JNDI_USER_TRANSACTION,
                jndiTransactionName);
